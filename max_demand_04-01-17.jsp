...
<table boarder="0" width="100%" id="max_demand">
    <tr>
        <!--        <td class="td1" >Billing Date</td>-->
        <%
            for (int i = 0; i < data_string2.length; i++) {
        %>
        <td class="td1"><%=data_string2[i]%></td>
        <td class="td1">Occured Date</td>
        <%}
    ...
    <%
        ...
        if (!fromDate.equals("") && !toDate.equals("")) {
            if (DBUtil.getTargetDB().equals("oracle")) {
                sql = "select count(distinct substr(data_string,0,instr(data_string,',',1))) from d3_b5 where  meter_serial_no='" + meter_sl_no + "' and "
            ...
            sql = "select count(distinct substr(data_string,0,instr(data_string,',',1))) from d3_b5 where data_string IS NOT NULL  and meter_serial_no='" + meter_sl_no + "' order by d3_time_stamp desc ";
        }
        ...
        String testValue = "";
        while (rs.next()) {			
            String occDate = rs.getString("billing_date");                            
            String occ_date[] = occDate.split(",");
            String temp = occ_date[0];
            int dd = Integer.parseInt(temp.substring(0, temp.indexOf("-")));
            int mon = Integer.parseInt(temp.substring(temp.indexOf("-")+1, temp.lastIndexOf("-")));
            int yy = Integer.parseInt(temp.substring(temp.lastIndexOf("-")+1,temp.indexOf(" ")));            
            if((dd > 0 && dd <= 24) && (mon > 0 && mon <=12) && (yy > 2000 && yy < 2020) )
            {                
