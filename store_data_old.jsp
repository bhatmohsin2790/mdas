...
%>
 <%
    ...
    sql = "select distinct substr(d4_date,1,10) d4_date, substr(data_string,1,instr(data_string, ',', 1, 1)-1) PARAM1," +
            ...
            " TIME_DELAY from d4_data d4 where d4.cdfid in (select cdfid from meter_data_time_stamp where meterid=?)and to_date(substr(d4_date,1,10),'dd/mm/yyyy')>=to_date('"+lsdate+"','dd/mm/yyyy') and to_date(substr(d4_date,1,10),'dd/mm/yyyy')<=to_date('"+ledate+"','dd/mm/yyyy') and data_string is NOT NULL order by to_timestamp(d4_date,'dd/mm/yyyy hh24:mi:ss'),TIME_DELAY  ";
    }
                 else
                     {
