...
<%
           ...
           if(locationName.toLowerCase().contains("400kv"))
               lineCurrentsLimit=520;
          else if(locationName.toLowerCase().contains("250kv"))
               lineCurrentsLimit=325;
          else if(locationName.toLowerCase().contains("100kv"))
               lineCurrentsLimit=130;
          else if(locationName.toLowerCase().contains("63kv"))
               lineCurrentsLimit=80;
           else
               lineCurrentsLimit=0;
           
           if(lineCurrentsLimit==0){
            %>   
            <h3 style="color:red;text-align: center;">Site Name does not contains Load value...</h3>
           <%}else{
...
<table bordercolor="white" border="0" width="100%" style="font-size:11px;background-color:white;" id="dtable" >
    ...
    <tbody >
        <%
            ...
            double currentsLimit=lineCurrentsLimit/cmf;
            System.out.println(lineCurrentsLimit+"\t"+cmf+"\t"+currentsLimit);
            ...
            if (DBUtil.getTargetDB().equals("oracle")) {
                sql = "SELECT g3,rph_voltage,yph_voltage,bph_voltage,rph_line_current, "
                        ...
                        + " BPH_REACT_CURRENT,YPH_REACT_CURRENT,meter_time_stamp,log_time_stamp  "
                        ...
                        + "to_TIMESTAMP(g2,'dd-mm-yyyy hh24:mi:SS') <= to_date('" + edate + " 23:59:00','YYYY-mm-DD hh24:mi:ss') ) and (rph_line_current>"+currentsLimit+" or yph_line_current>"+currentsLimit+" or bph_line_current>"+currentsLimit+")"
        ...
        <%
            ...
            if(flag==0){
          %>
          <tr class="trbg"><td  colspan="15" style="text-align: center">No Over Load Data</td></tr>
           <%}   
