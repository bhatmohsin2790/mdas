    ...
    try {
       ...
       PreparedStatement pstmt = con.prepareStatement(sql11);
        ...
        for (int i = 0; i < f_ids_arr.length; i++) {
            ...
            pstmt = con.prepareStatement("select * from d4_columns_header where meter_serial_no=(select meter_serial_no from meter where meterid=?)");
            pstmt.setString(1, meter_ids_arr[i]);
            rs = pstmt.executeQuery();
            if (rs.next()) {
                System.out.println("---------------------------Load survay Header Got Dynamically-----------------------------------");
                param_names = rs.getString("load_columns_header").split(",");
            } else {
                System.out.println("---------------------------Load survay Header Got Manually-----------------------------------");
                pstmt = con.prepareStatement("select no_of_params,d4_params from metermake where metermakeid=(select metermake from meter where meterid=?)");
                pstmt.setString(1, meter_ids_arr[i]);
                rs = pstmt.executeQuery();
                if (rs.next()) {
                    param_names = rs.getString("d4_params").split(",");
                }//end if
            }//end else
%>
...
<table border="0" width="100%" id="table_disp" cellpadding="0" cellspacing="1px" style="float:left;margin-right: 10px;<% if (i >= 2) {
    ...
    <thead>
        ...
        <tr>
            ...
            <%
               ...
               { %>
               <td width="60px" align="center">Fwd.kVAh</td>
               <TD width="60px" align="center">Fwd.kWh</td>
               <td width="60px" align="center">Fwd.kVArh Lag</td>
               <td width="60px" align="center">Fwd.kVArh Lead</td>
               <td width="60px" align="center">RV</td>
               <TD width="60px" align="center">YV</td>
               <TD width="60px" align="center">BV</TD>
               <TD width="60px" align="center">RI</TD>
               <TD width="60px" align="center">YI</TD>
               <TD width="60px" align="center">BI</TD>
          <%     }
                       else{
    ...
    <tbody>
        ...
        if (DBUtil.getTargetDB().equals("mysql")) {
            pstmt = con.prepareStatement("select * from d4_data where meter_serial_no=? and data_string is not null order by d4_time_stamp desc limit 10");
                       }
            if (DBUtil.getTargetDB().equals("oracle")) {
            pstmt = con.prepareStatement("select * from(select cdfid,d4_date,time_delay,"
                    + "data_string,meter_serial_no,d4_time_stamp,rownum from "
                    + "d4_data where meter_serial_no=? and data_string is not null order by "
                    + "d4_time_stamp desc)where rownum<=10" );
                       }
            ...
            while (rs.next()) {
                ...
                try {
                        for (int j = 0; j < data_string_arr.length; j++) {
                                df1.format(Double.parseDouble(data_string_arr[j]));
                        }
                    out.println("<tr>");
                    out.println("<td align='left' width='60px'>" + rs.getString("d4_date").substring(0, 10) + "</td>");
                    out.println("<td align='left' width='60px'>" + rs.getString("d4_date").substring(10) + "</td>");
                    for (int j = 0; j < data_string_arr.length; j++) {
